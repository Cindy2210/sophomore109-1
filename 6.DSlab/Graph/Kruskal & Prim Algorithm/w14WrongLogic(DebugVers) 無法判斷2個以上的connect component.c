...
void Kruskal(struct Vertex v[10], struct Edge e[100], int edgeLength, struct Edge *result[100])
{
	...
	for (int l = 0; l < edgeLength; l++)
	{
		...
		//		printf("(%p)%d \t(%p)%d\t", e[min_i].end[0], e[min_i].end[0]->id, e[min_i].end[1], e[min_i].end[1]->id);
		//		printf("w:%d(%p)\n", e[min_i].w, e[min_i]);
		//		printf("\n---------------------------------------------------------\n");
	...
	for (int j = 0; j < edgeLength; j++)
	{
		...
		if (isForest(visited))
		{
			...
			//result[tmpLength]->w = sort[j].w;

			tmp[tmpLength].end[0] = sort[j].end[0];
			//result[tmpLength]->end[0] = sort[j].end[0];

			tmp[tmpLength].end[1] = sort[j].end[1];
			//result[tmpLength]->end[1] = sort[j].end[1];
			tmpLength++;
		}
		else
		{
			//visited[(sort[j].end[0]->id) - 1].edgeList[visited[(sort[j].end[0]->id) - 1].edgeCount] = NULL;
			visited[(sort[j].end[0]->id) - 1].edgeCount--;
			//visited[(sort[j].end[1]->id) - 1].edgeList[visited[(sort[j].end[1]->id) - 1].edgeCount] = NULL;
			visited[(sort[j].end[1]->id) - 1].edgeCount--;
		}
	}
	printf("==============================================================\n\n");
...
int isTreePrim(struct Vertex v[10], int root_i, struct Vertex setU[10])
{
}
